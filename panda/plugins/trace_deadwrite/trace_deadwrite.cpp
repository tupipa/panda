...
//         printf("get currentTraceShadowIp from gBlockShadowMap[callerIp]\n");
//     	target_ulong * currentTraceShadowIP = (target_ulong *) gBlockShadowMap[callerIp];
...
VOID InitContextTree(){
    ...
    printf("initialize gBlockShadowMap and gTraceShadowMapIps\n");
        //uint32_t traceSize = TRACE_Size(trace);    
...
// give space to account for nSlots which we record later once we know nWrites
    ...
    *pNumWrites = slot;
    ...
    printf("BUG HERE\n");
    printf("%s: size of (unordered_map<ADDRINT,bool>): %d\n", __FUNCTION__,(int)sizeof(unordered_map<ADDRINT,bool>) );
    unordered_map<ADDRINT,bool> * mapIps = 
        new unordered_map<ADDRINT, bool>;
    ...
    printf("done. initialize gBlockShadowMap and gTraceShadowMapIps\n");
...
//#endif //end  ifndef IP_AND_CCT         
    ...
    if (! is_write){
        ...
        if (gBlockShadowMapDone[gCurrentTraceBlock->address]){
            ...
            // printf("%s: now check gCurrentTraceIpVector\n", __FUNCTION__);
            // if(gCurrentTraceIpVector[slot] != gCurrentTraceBlock){
            //     printf("%s: ERROR: should set gCurrentTraceIpVector[slot] != gCurrentTraceBlock.\n", __FUNCTION__);
            // }
        ...
        //Third, update gCurrentTraceIpVector if a new Trace.
        ...
        //target_ulong * currentTraceShadowIP = (target_ulong *) gBlockShadowMap[gCurrentContext->address];
...
inline void InitializeBlockShadowMap(CPUState *cpu, TranslationBlock *tb){
    ...
    if(needUpdate){
        ...
        *pNumWrites = slot;
        ...
        //lele: gTraceShadowMapIps;
        unordered_map<ADDRINT,bool> * mapIps = new unordered_map<ADDRINT,bool> ;
            //(unordered_map<ADDRINT, bool> *) malloc (sizeof(unordered_map<ADDRINT,bool>));
        (*mapIps)[traceAddr]=false;
        gTraceShadowMapIps[traceAddr] = mapIps;

        printf("%s: set gBlockShadowMap[0x" TARGET_FMT_lx "] as false for this block\n", __FUNCTION__, tb->pc);
    ...
    if( (gTraceIter = (gCurrentContext->childBlocks).find(currentIp)) != gCurrentContext->childBlocks.end()) {
        ...
        if(recordedSlots >0 ){
             ...
             if (gBlockShadowMapDone[tb->pc]){
                ...
                //printf("%s: old gBlockShadowMap is not used: tb->pc=0x" TARGET_FMT_lx ".\n", __FUNCTION__, tb->pc);
            }
